...
public class ChatServerWorker extends Thread implements MessageTypes {
	...
	public void run() {
		...
		//System.out.println("near to TRY");
		try {
			...
			//System.out.println("near to msg");
		}
		...
		switch(message.getType()) {
			...
			case NOTE:
				...
				while(participantsIterator.hasNext()) {
					...
					//try to connect to one client at a time
					try {
						System.out.println(participantInfo.getIp() +
								 participantInfo.getPort()); //m
						chatConnection = new Socket( participantInfo.getIp(),
								 participantInfo.getPort()); 
						//System.out.println("after chat");
						//write message of type note to client
						writeToNet.writeObject( message );
						//System.out.println("after writeToNet");
						//close connection
						chatConnection.close();
					}
