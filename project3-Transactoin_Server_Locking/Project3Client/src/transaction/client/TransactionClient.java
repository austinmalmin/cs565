...
public class TransactionClient implements MessageTypes
{
	...
	public static void main( String[] args)
	{
		...
		for(int current_transaction = 0; current_transaction < tclient.total_transctions; current_transaction++) {
			...
			System.out.println("account:"+first_account+" taking " + balance_change + " from second account :" + second_account);
		...
		int local_abortedIndex = 0;
		 ...
		 while(!aborted_transactionInfo.isEmpty()) {
		  ...
		  List<Integer> transInfo = (List<Integer>)
		  aborted_transactionInfo.get(local_abortedIndex); 
		  ...
		  int Bal1 = proxy.read(first_account, transactionId); int Bal2 =
		  proxy.read(second_account, transactionId);
		  
		  Bal1 += balance_change; Bal2 -= balance_change;
		  
		  proxy.write(first_account, Bal1, transactionId); proxy.write(second_account,
		  Bal2, transactionId);
		  ...
		  transactionInfo.add(first_account); transactionInfo.add(second_account);
		  transactionInfo.add(balance_change);
		  
		  aborted_transactionInfo.put(abortedIndex, transactionInfo); abortedIndex++;
		  
		  }
		  
		  //removing aborted transaction at the end
		  aborted_transactionInfo.remove(local_abortedIndex); local_abortedIndex++; }
		 
		
		
		 TransactionServerProxy proxy = new TransactionServerProxy(tclient.serverIP, tclient.serverPort); 
		 int sum = 100;
		 System.out.println("Expected sum is " + (tclient.startingBal * tclient.numAccounts) + ", sum is " + sum);
